...
## 7. AI 세션 시작 시 필독 순서 (⚠️ 작업 전 반드시 확인)

작업 시작 전 아래 순서대로 파일을 읽어 컨텍스트를 파악하세요.

1. `.workzones.yml` — 현재 locked 구역 확인 (충돌 방지)
2. `work-logs/` 최신 파일 3개 — 다른 팀원의 최근 작업 파악
3. 해당 기능의 `MAP.md` — 관련 파일 목록 빠르게 파악 (있는 경우)

> 위 3개 파일만 읽어도 대부분의 세션 컨텍스트가 확보됩니다.
> CLAUDE.md 전체, DEV_ROADMAP.md 등은 필요한 경우에만 읽으세요.

---

## 8. 현재 진행 중인 작업 현황 (⚠️ 작업 시작 전 반드시 확인)

> 이 표는 `.workzones.yml` 기반으로 관리됩니다.
> 작업 시작 시 `.workzones.yml`에 구역을 선언하고, 이 표를 업데이트하세요.
> PR 머지 후에는 반드시 해당 항목을 제거하세요.
