...
| 작업 커맨드 | `/spm-start`, `/spm-commit`, `/spm-done` 등 스킬 자동 실행 | `.agents/skills/spm-*` 스킬 또는 이 가이드 수동 절차 |
| Linear 연동 | MCP 서버로 직접 조회/생성/업데이트 | Linear 웹 UI 또는 Linear CLI 사용 |
| 디자인 스킬 | `/critique`, `/polish` 등 21개 UI/UX 스킬 | `.agents/skills/*/SKILL.md` 기준으로 동일 적용 |
| GitHub 연동 | Claude GitHub 도구/MCP | Codex Apps GitHub MCP 사용 가능 |
...
### 2.5 Codex 스킬 디렉토리 확인

Codex는 프로젝트의 `.agents/skills/`를 읽습니다. 아래 스킬들이 존재하는지 확인하세요:
...
Codex 기준으로는 `.agents/skills/[스킬명]/SKILL.md`를 사용합니다.
각 스킬의 분석 기준과 체크리스트가 정의되어 있으므로, 해당 내용을 참고하여 수동으로 적용할 수 있습니다.

예시:
```
.agents/skills/critique/SKILL.md      → UX 평가 기준
.agents/skills/polish/SKILL.md        → 최종 품질 체크리스트
.agents/skills/audit/SKILL.md         → 접근성/성능 감사 기준
```
...
현재 이 프로젝트 기준으로는 Linear MCP 직접 호출을 사용하지 않습니다.
Linear 웹 UI 또는 Linear CLI를 사용하고, PR 본문에 `Linear: SPM-[번호]`를 넣어 자동 연동하세요.
