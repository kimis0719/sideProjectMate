...
> **원칙: PC와 동일한 동작.** 크기만 풀스크린으로 변경되며, 플로우·애니메이션·기능은 PC와 같다.

| 항목 | 정책 |
|------|------|
| 진입 | 헤더 채팅 아이콘 클릭 → 미니 위젯 열림 (PC와 동일) |
| 애니메이션 | PC와 동일 (opacity + translateY) |
| 플로우 | 위젯 오픈 → 룸 리스트(미니) → 채팅 입장(미니) |
| 브라우저 뒤로가기 | 위젯 닫힘 (history.pushState로 상태 관리) |
| 스와이프로 닫기 | 1차 구현에서 제외, 테스트 후 필요성 판단 |
| `[↗]` 풀페이지 확장 | PC와 동일하게 구현 |
| 백드롭(dim) | PC와 동일 (위젯 바깥 클릭 시 닫힘) |
