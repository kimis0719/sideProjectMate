...
const MenuBar = ({ editor }: { editor: any }) => {
  ...
  return (
    <div className="border-b border-border p-2 flex flex-wrap gap-2 sticky top-0 bg-background z-10">
      ...
      <button onClick={addImage} className="px-2 py-1 rounded hover:bg-muted/50">
        Image
      </button>
    </div>
  );
...
export default function BlockEditor({ content, onChange, editable = true }: BlockEditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'prose dark:prose-invert prose-sm sm:prose lg:prose-lg xl:prose-2xl mx-auto focus:outline-none min-h-[300px] p-4 text-foreground',
      },
    },
  });
