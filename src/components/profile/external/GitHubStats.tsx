...
export default function GitHubStatsSection({ githubUrl }: GitHubStatsProps) {
    ...
    return (
      <div className="bg-surface-container-lowest rounded-xl p-6 lg:p-10 shadow-sm flex flex-col items-center justify-center min-h-[300px] text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          GitHub 계정을 연결하면 활동 분석과 기여도를 확인할 수 있습니다.
        </p>
      </div>
    );
  }
  if (loading)
    return (
      <div className="bg-surface-container-lowest rounded-xl p-6 lg:p-10 shadow-sm flex items-center justify-center min-h-[300px]">
    ...
    <div className="bg-surface-container-lowest rounded-xl p-6 lg:p-10 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div className="flex flex-col items-center justify-center p-4 bg-gradient-to-br from-indigo-50/50 to-purple-50/50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-xl border border-indigo-100 dark:border-indigo-500/30">
          ...
          <div className="text-4xl font-extrabold text-indigo-900 dark:text-indigo-300 mb-1">
          ...
          <div className="w-full grid grid-cols-2 gap-2 text-sm">
            <div className="bg-card p-2 rounded text-center">
              ...
              <div className="font-bold text-foreground">
                {stats.activity.totalCommits.toLocaleString()}
              </div>
            </div>
            <div className="bg-card p-2 rounded text-center">
        ...
        <div className="flex flex-col h-full">
          <div className="text-sm text-muted-foreground font-semibold mb-2 uppercase tracking-wider">
            Top Skills via Contribution
          </div>
            ...
            {(() => {
              ...
              return topSkills.map((tier) => {
                ...
                return (
                  <div
                    key={tier.name}
                    className="flex items-center justify-between bg-card p-2 rounded-lg border border-border hover:shadow-md transition-shadow"
                  >
                    ...
                    <div className="flex flex-col items-end">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-bold w-6 text-center 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div>
            <div className="text-sm text-muted-foreground font-semibold mb-2 flex justify-between items-center">
              ...
              <span className="text-xs font-normal bg-muted px-2 py-0.5 rounded text-foreground">
                Last 16 Weeks
              </span>
            </div>
          ...
          {/* Most Active Repos */}
          <div>
            <div className="text-sm text-muted-foreground font-semibold mb-3 uppercase tracking-wider">
            ...
            <div className="space-y-2">
              {stats.topRepos && stats.topRepos.length > 0 ? (
                stats.topRepos.slice(0, 2).map((repo, idx) => (
                  <div
                    key={idx}
                    className="bg-muted/30 p-2.5 rounded-lg border border-border/50 flex justify-between items-center"
                  >
