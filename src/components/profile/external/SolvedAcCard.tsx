...
const getTierName = (tier: number) => {
  ...
  switch (tierGroup) {
    case 1:
      return `Bronze ${romanLevel}`;
    case 2:
      return `Silver ${romanLevel}`;
    case 3:
      return `Gold ${romanLevel}`;
    case 4:
      return `Platinum ${romanLevel}`;
    case 5:
      return `Diamond ${romanLevel}`;
    case 6:
      return `Ruby ${romanLevel}`;
    default:
      return 'Master';
  }
};
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: '0.875rem', color: 'var(--muted-foreground)' }}>
          백준(BOJ) 핸들을 등록하여 알고리즘 문제 해결 능력을 증명하세요.
        </p>
      </CardContainer>
    );
  }
  ...
  return (
    <CardContainer>
      ...
      <div className="flex flex-col items-center py-2">
        ...
        <TierBadge $color={getTierColor(data.tier)}>⭐ Rating {data.rating}</TierBadge>
      </div>
      ...
      {data.problemStats && totalCalculated > 0 && (
        <DistributionContainer>
          ...
          <DistributionBar>
            {statsByTierGroup.Bronze > 0 && (
              <DistributionSegment
                $color="#ad5600"
                $width={(statsByTierGroup.Bronze / totalCalculated) * 100}
              />
            )}
            {statsByTierGroup.Silver > 0 && (
              <DistributionSegment
                $color="#435f7a"
                $width={(statsByTierGroup.Silver / totalCalculated) * 100}
              />
            )}
            {statsByTierGroup.Gold > 0 && (
              <DistributionSegment
                $color="#ec9a00"
                $width={(statsByTierGroup.Gold / totalCalculated) * 100}
              />
            )}
            {statsByTierGroup.Platinum > 0 && (
              <DistributionSegment
                $color="#27e2a4"
                $width={(statsByTierGroup.Platinum / totalCalculated) * 100}
              />
            )}
            {statsByTierGroup.Diamond > 0 && (
              <DistributionSegment
                $color="#00b4fc"
                $width={(statsByTierGroup.Diamond / totalCalculated) * 100}
              />
            )}
            {statsByTierGroup.Ruby > 0 && (
              <DistributionSegment
                $color="#ff0062"
                $width={(statsByTierGroup.Ruby / totalCalculated) * 100}
              />
            )}
          </DistributionBar>
          <DistributionLegend>
            {statsByTierGroup.Bronze > 0 && (
              <LegendItem $color="#ad5600">Bronze ({statsByTierGroup.Bronze})</LegendItem>
            )}
            {statsByTierGroup.Silver > 0 && (
              <LegendItem $color="#435f7a">Silver ({statsByTierGroup.Silver})</LegendItem>
            )}
            {statsByTierGroup.Gold > 0 && (
              <LegendItem $color="#ec9a00">Gold ({statsByTierGroup.Gold})</LegendItem>
            )}
            {statsByTierGroup.Platinum > 0 && (
              <LegendItem $color="#27e2a4">Platinum ({statsByTierGroup.Platinum})</LegendItem>
            )}
            {statsByTierGroup.Diamond > 0 && (
              <LegendItem $color="#00b4fc">Diamond ({statsByTierGroup.Diamond})</LegendItem>
            )}
            {statsByTierGroup.Ruby > 0 && (
              <LegendItem $color="#ff0062">Ruby ({statsByTierGroup.Ruby})</LegendItem>
            )}
          </DistributionLegend>
        </DistributionContainer>
      )}
