...
export default function AdminProjectDetailModal({ pid, onClose, onUpdated, onDeleted }: Props) {
  ...
  const statusInfo = project ? (STATUS_MAP[project.status] ?? { label: project.status, color: 'bg-muted text-muted-foreground' }) : null;
    ...
    <>
      ...
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 py-5">
          ...
          {!loading && !project && (
            <div className="text-center py-12 text-muted-foreground text-sm">프로젝트를 찾을 수 없습니다.</div>
          )}
            ...
            <div className="space-y-5">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-1.5">
                  <span className="text-xs font-mono text-muted-foreground">PID: {project.pid}</span>
                  {statusInfo && (
              ...
              <div className="border border-border rounded-lg divide-y divide-border">
                ...
                <InfoRow label="생성일" value={new Date(project.createdAt).toLocaleDateString('ko-KR')} />
                <InfoRow label="최종 수정" value={new Date(project.updatedAt).toLocaleDateString('ko-KR')} />
              </div>
              ...
              <div>
                ...
                {project.author ? (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-foreground">{project.author.nName}</p>
                      <p className="text-xs text-muted-foreground">{project.author.authorEmail}</p>
                    </div>
                    <button
