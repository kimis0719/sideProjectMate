...
export function generateAdjustmentSuggestions(
  ...
  // 2. 병렬 진행 제안 (경고만 표시)
  const totalOverlapDays = conflicts.reduce((sum, conflict) => sum + conflict.overlapDays, 0);
 ...
 */
export function checkAllScheduleConflicts(tasks: ConflictTask[]): Map<string, ScheduleConflict[]> {
