 ...
 */
export function identifyParallelTasks(tasks: TaskDependency[]): Map<string, TaskDependency[]> {
  const parallelGroups = new Map<string, TaskDependency[]>();

  for (const task of tasks) {
    const predecessors = getPredecessorTasks(task, tasks);
    const parallelPreds = predecessors.filter(({ type }) => type === 'SS' || type === 'FF');
 ...
 */
export function identifySerialTasks(tasks: TaskDependency[]): Array<TaskDependency[]> {
...
export function detectCircularDependency(
  ...
  for (const { task: successor } of successors) {
    const result = detectCircularDependency(successor, allTasks, new Set(visited), [...path]);
 ...
 */
export function calculateCriticalPath(tasks: TaskDependency[]): TaskDependency[] {
