...
function ProjectList() {
  ...
  if (isLoading) return <div className="text-center py-20">프로젝트를 불러오는 중... 🚀</div>;
    ...
    <div className="bg-white">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="w-full md:w-96">
              <div className="relative">
                <input type="text" placeholder="제목 또는 기술 스택으로 검색" className="w-full px-4 py-3 pl-12 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      ...
      <section className="container mx-auto px-4 py-12">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href="/projects/new" className="bg-gray-900 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-800 transition-colors">+ 새 프로젝트 만들기</Link>
        </div>
        {filteredProjects.length > 0 ? (
          ...
          <div className="text-center py-20"><p className="text-gray-500">찾으시는 조건의 프로젝트가 없습니다. 🥲</p></div>
        )}
        <div className="text-center mt-12">
          <div className="inline-flex items-center space-x-2">
            <button className="px-4 py-2 border rounded-lg text-gray-600 hover:bg-gray-100">이전</button>
            <button className="px-4 py-2 border rounded-lg text-white bg-gray-800">1</button>
            ...
            <button className="px-4 py-2 border rounded-lg text-gray-600 hover:bg-gray-100">다음</button>
          </div>
        </div>
      </section>
    </div>
...
export default function ProjectsPage() {
  return (
    <Suspense fallback={<div className="text-center py-20">페이지를 불러오는 중...</div>}>
      <ProjectList />
    </Suspense>
  );
}
