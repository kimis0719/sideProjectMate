    ...
    <div className="flex min-h-[calc(100vh-64px)] relative bg-background">
      ...
      <aside
        className={`
                ...
                `}
      >
        <SidebarContent pid={pid} isActive={isActive} onClose={() => setIsSidebarOpen(false)} />
      </aside>
      ...
      {/* 4. 메인 컨텐츠 */}
      <main className={`flex-1 relative w-full ${isHomePage ? 'min-w-0' : ''}`}>{children}</main>
    </div>
  );
}
