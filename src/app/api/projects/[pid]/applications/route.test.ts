...
describe('GET /api/projects/[pid]/applications', () => {
  ...
  it('프로젝트 작성자가 지원자 목록을 조회한다', async () => {
    ...
    const request = new Request('http://localhost:3000/api/projects/1/applications');
    ...
    expect(body.data[0].applicantId).toHaveProperty('nName', '지원자');
  });
  ...
  it('작성자가 아닌 유저는 403을 반환한다', async () => {
    ...
    mockGetServerSession.mockResolvedValue({
      user: { _id: otherUser._id.toString() },
      expires: '2099-12-31',
    });

    const request = new Request('http://localhost:3000/api/projects/2/applications');
  ...
  it('존재하지 않는 프로젝트면 404를 반환한다', async () => {
    ...
    const request = new Request('http://localhost:3000/api/projects/9999/applications');
  ...
  it('지원자가 없으면 빈 배열을 반환한다', async () => {
    const owner = await createTestUser();
    const project = await createTestProject(owner._id, 3);
    ...
    const request = new Request('http://localhost:3000/api/projects/3/applications');
    const response = await GET(request, { params: { pid: '3' } });
    const body = await response.json();
