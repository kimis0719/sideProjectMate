  ...
  try {
    ...
    if (!resourceId) {
      return NextResponse.json(
        { success: false, message: 'Resource ID가 필요합니다.' },
        { status: 400 }
      );
    }
    ...
    if (!resource) {
      return NextResponse.json(
        { success: false, message: '리소스를 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
    ...
    if (!isProjectAuthor && !isResourceOwner) {
      return NextResponse.json(
        { success: false, message: '수정 권한이 없습니다. (본인이 등록한 자산만 수정 가능)' },
        { status: 403 }
      );
    }
...
async function handlePost(request: Request, { params }: { params: { pid: string } }) {
  ...
  try {
    ...
    if (!isProjectAuthor && !isMember) {
      return NextResponse.json(
        { success: false, message: '프로젝트 멤버만 자산을 등록할 수 있습니다.' },
        { status: 403 }
      );
    }
...
async function handleDelete(request: Request, { params }: { params: { pid: string } }) {
  ...
  try {
    ...
    if (!resource) {
      return NextResponse.json(
        { success: false, message: '이미 삭제되었거나 존재하지 않는 리소스입니다.' },
        { status: 404 }
      );
    }
    ...
    if (!isProjectAuthor && !isResourceOwner) {
      return NextResponse.json(
        { success: false, message: '삭제 권한이 없습니다. (본인이 등록한 자산만 삭제 가능)' },
        { status: 403 }
      );
    }
