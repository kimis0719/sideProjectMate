...
export const useWbsStore = create<WbsState>()(
  devtools((set, get) => ({
    ...
    addTask: async (task: Omit<Task, 'id'>) => {
      ...
      try {
        ...
        set((state) => ({
          ...
          selectedTaskId:
            state.selectedTaskId === optimisticTask.id ? savedTask.id : state.selectedTaskId,
        }));
    ...
    updateTaskDates: async (taskId: string, startDate: Date, endDate: Date) => {
      ...
      // Optimistic Update: 즉시 UI에 반영
      set((state) => ({
        tasks: state.tasks.map((t) => (t.id === taskId ? { ...t, startDate, endDate } : t)),
      }));
